...
async def post_number_of_comments(loop, session, post_id, task_registry):
    ...
    tasks = [post_number_of_comments(
        loop, session, kid_id, task_registry
    ...
    if number_of_comments > MIN_COMMENTS:
        task_registry.append(asyncio.create_task(log_post(response)))
...
async def main(loop, post_id, task_registry):
...
if __name__ == '__main__':
    ...
    comments = loop.run_until_complete(main(loop, post_id, task_registry))
