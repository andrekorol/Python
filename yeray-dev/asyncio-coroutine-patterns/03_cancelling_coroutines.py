...
async def post_number_of_comments(loop, session, fetcher, post_id):
    ...
    try:
        ...
        tasks = [asyncio.create_task(post_number_of_comments(
        ...
        try:
            ...
            log.debug('Error retrieving comments for top stories: '
                      '{}'.format(e))
            raise
        ...
        if tasks:
            log.info('Comments for post {} cancelled, cancelling {} '
                     'child tasks'.format(
                         post_id, len(tasks)))
            for task in tasks:
                task.cancel()
    ...
    while True:
        ...
        def callback(fut, errors):
            try:
                ...
                log.info(
                    '> Calculating comments took {:.2f} seconds and {} '
                    'fetches'.format(
                        (datetime.now() - now).total_seconds(), fetch_count))

        task.add_done_callback(partial(callback, errors=errors))
...
async def main(loop, period, limit):
    ...
    async with aiohttp.ClientSession(loop=loop) as session:
        comments = await poll_top_stories_for_comments(loop, session,
                                                       period, limit)
